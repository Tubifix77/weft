...
1. **Token economy.** Keywords, multi-character names, indentation, and most punctuation exist for a human. None of it does anything for an interpreter. With no reader, all of it becomes pure overhead to be deleted — fewer output tokens per unit of work, which is directly cheaper and faster to generate. **[MEASURED FALSE, 2026-07-01 — see `MEASUREMENT.md`.** Weft costs ~3.6x *more* tokens than idiomatic Python, tokenizer-robust. The flat-SSA overhead — explicit handles, one op per line, typed-literal ops, BPE-unfriendly mnemonics — dwarfs the keyword savings, and small idiomatic Python has little boilerplate to delete in the first place. This claim is refuted.]**
...
4. **Tokenizer fit — MEASURED, claim refuted.** The opcode mnemonics were *intended* to be single tokens; in practice, against three stock code-model tokenizers (qwen2.5-coder, phi4, gemma3) they fragment, and the whole program runs ~3.6x more tokens than idiomatic Python (`MEASUREMENT.md`). Single-token mnemonics would require a custom/adapted tokenizer, which conflicts with the untouched-model + constrained-decoding design (§7). The token-economy reason-to-exist (§1) does not hold.
